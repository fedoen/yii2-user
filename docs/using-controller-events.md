...
```php
'user' => [
    'class' => \fedoen\user\Module::className(),
    'controllerMap' => [
        'registration' => [
            'class' => \fedoen\user\controllers\RegistrationController::className(),
            'on ' . \fedoen\user\controllers\RegistrationController::EVENT_AFTER_REGISTER => function ($e) {
...
```php
\yii\base\Event::on(
    \fedoen\user\controllers\RegistrationController::className(),
    \fedoen\user\controllers\RegistrationController::EVENT_AFTER_CONFIRM,
    function () {        
